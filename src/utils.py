...
def resolve_file(candidates: Sequence[str], roots: Optional[Iterable[Path]] = None) -> Path:
    """
...
def eda_overview(df: pd.DataFrame, target_col: str | None = None, top_k_missing: int = 10) -> None:
    """
...
def plot_numeric_histograms(df: pd.DataFrame, numeric_cols: List[str], bins: int = 30, max_cols: int = 8) -> None:
    """
...
def plot_corr_heatmap(df: pd.DataFrame, numeric_cols: List[str], max_cols: int = 15) -> None:
    """
...
class RareCategoryGrouper(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        for c in self.columns_:
            if c in X.columns:
                X[c] = X[c].astype("object").where(X[c].isin(self.levels_[c]), other=self.token)
        return X
...
def build_preprocessor(numeric: List[str], categorical: List[str], rare_min_freq: int = 30) -> ColumnTransformer:
    """
...
def build_models(
    ...
    def wrap(model):
        ...
        if HAS_IMB and use_smote:
            steps.append(("smote", SMOTE(k_neighbors=smote_k_neighbors, random_state=seed)))
        final_estimator = _wrap_with_calibration(model, method=calibration, cv=3) if calibrate else model
        steps.append(("model", final_estimator))
        return ImbPipeline(steps) if (HAS_IMB and use_smote) else Pipeline(steps)

    models: Dict[str, Pipeline] = {
        "LogReg": wrap(LogisticRegression(C=1.5, max_iter=800, class_weight="balanced")),
        "RF": wrap(
...
def cv_summary_for_models(
    ...
    df = pd.DataFrame(rows).sort_values(["pr_auc_mean", "roc_auc_mean"], ascending=False).reset_index(drop=True)
    return df
...
def choose_best_name(cv_df: pd.DataFrame) -> str:
    ...
    """
    return cv_df.sort_values(["pr_auc_mean", "roc_auc_mean"], ascending=False).iloc[0]["model"]


def holdout_metrics(y_true: np.ndarray, y_prob: np.ndarray, threshold: float = 0.5) -> Dict[str, float]:
    """
...
def permutation_importance_df(
    ...
    perm = permutation_importance(pipeline, X_s, y_s, n_repeats=n_repeats, random_state=random_state, n_jobs=-1)
...
def adversarial_auc(preprocessor, X_train, X_test, seed: int = 42) -> float:
    ...
    prob = cross_val_predict(pipe, adv_X, adv_y, cv=cv, method="predict_proba", n_jobs=-1)[:, 1]
...
def clean_and_merge_strict(
    ...
    if ("Project code" in d.columns) and ("Project code" in df_comp.columns):
        comp_trim = df_comp.drop(
            columns=[c for c in ["Initiator", "Link to Portal"] if c in df_comp.columns],
            errors="ignore",
        )
    ...
    if {"Committed funding, ₴", "Overall estimated budget, ₴"}.issubset(d.columns):
        ...
        d["Financial_Efficiency"] = (d["Committed funding, ₴"] / denom).replace([np.inf, -np.inf], np.nan)
