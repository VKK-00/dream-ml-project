...
def main():
    ...
    mask = df[cfg["features"]["target_col"]].isin(cfg["features"]["target_mapping"].keys())
    df_f = df.loc[mask].copy()
    df_f["Project_Outcome"] = df_f[cfg["features"]["target_col"]].map(cfg["features"]["target_mapping"]).astype(int)
    ...
    pre = build_preprocessor(numeric, categorical, rare_min_freq=cfg["ml"]["rare_min_freq"])
    models = build_models(
        pre,
    ...
    imp.to_csv(artifacts / "feature_importance_top25.csv", index=False, encoding="utf-8")
